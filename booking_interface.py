...
class BookingPage(QMainWindow):
    ...
    def __init__(self):
        ...
        # self.ui.cal_check_in.clicked[QtCore.QDate].connect(self.showDate)
        self.ui.view_availability.clicked.connect(lambda: self.check_availiability(self.ui.ledit_city_name.text(), self.ui.ledit_hotel.text()))
        self.window = Second(self)
        self.init_ui()
    ...
    def check_availiability(self, fname, fcity):
            label = QLabel(self.window)
            label.setText(fname)
            label1 = QLabel(self.window)
            pixmap = QPixmap('jw.jpg')
            label1.setPixmap(pixmap)
            self.window.setCentralWidget(label1)
